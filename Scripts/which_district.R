...
check_coords_in_psnu <- function(shape_file, coordinate_file){
    ...
    # Combine longitude and latitude into a single column
    ...
    # Perform spatial join between districts and facilities
...
all_data <- check_coords_in_psnu(shape_file, coordinate_file) %>% 
    write_excel_csv("Dataout/all_data.csv")

wrong_psnu <- all_data %>% 
    filter(check_psnu == 1) %>% 
    write_excel_csv("Dataout/check_psnu_gis.csv")
